...
class PoissonProblem:
    ...
    def residual(self):
        s = np.pad(
            self.solution, [(1, 1), (1, 1)], mode="edge"
        )  # So that BC dp/dn = 0 holds
        res = (
            (s[0:-2, 1:-1] - 2.0 * s[1:-1, 1:-1] + s[2:, 1:-1]) / self.h_x ** 2
    ...
    def set_chebyshev_steps(self, batch_size: int = 5):
        ...
        l = 4 * (
            np.sin(np.pi / 2 / self.n_x) ** 2 / self.h_x ** 2
            + np.sin(np.pi / 2 / self.n_y) ** 2 / self.h_y ** 2
        )
        L = 4 * (
            np.cos(np.pi / 2 / self.n_x) ** 2 / self.h_x ** 2
            + np.cos(np.pi / 2 / self.n_y) ** 2 / self.h_y ** 2
        )
        mu = L / l
        n_iter = 2 ** batch_size
        _is = PoissonProblem.generate_chebyshev_index_sequence(batch_size)
        taus = 1.0 / (
            (L + l) / 2 + (L - l) / 2 * np.cos(np.pi * (2 * _is - 1) / 2 / n_iter)
        )
